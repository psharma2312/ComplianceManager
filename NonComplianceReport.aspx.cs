...
namespace ComplianceManager
{
	public partial class NonComplianceReport : System.Web.UI.Page
	{
        ...
        protected void gvCompliances_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            ...
            int priorityId = ViewState["ApproverId"] != null ? Convert.ToInt32(ViewState["ApproverId"]) : 0;

            gvCompliances.PageIndex = e.NewPageIndex;
            BindGridFiltered(deptId, typeId, priorityId);
        }
        ...
        protected void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            if (ddlTypeOfCompliance.SelectedIndex == 0 && ddlDepartmentFunction.SelectedIndex == 0 && ddlPriority.SelectedIndex == 0)
            {
                ShowMessage("Please select at least one from Department / Type / Priority.", true);
                return;
            }

            lblErrorMessage.Visible = false;
            BindGridFiltered(deptId, typeOfCompliance, priorityId);
        }
        ...
        private void BindGridFiltered(int deptId = 0, int typeId = 0, int priority = 0)
        {
            try
            {
                ...
                ViewState["PriorityId"] = priority;

                DAL dal = new DAL();
                List<ComplianceMasterr> compliances = new List<ComplianceMasterr>();
                compliances = dal.LoadNonCompliancesForReport(deptId, typeId, priority);
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            int deptId = ViewState["DeptId"] != null ? Convert.ToInt32(ViewState["DeptId"]) : 0;
            int typeId = ViewState["TypeId"] != null ? Convert.ToInt32(ViewState["TypeId"]) : 0;
            int priority = ViewState["PriorityId"] != null ? Convert.ToInt32(ViewState["PriorityId"]) : 0;

            DAL dal = new DAL();
            var varCatalogue = dal.LoadNonCompliancesForReport(deptId, typeId, priority);
            using (var workbook = new XLWorkbook())
            {
                ...
                string excelName = "NonComplianceReport_" + localTime.ToString("dd_MMM_yy");
                Response.Clear();
