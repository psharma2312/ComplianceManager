<%@ Page Title="" Language="C#" MasterPageFile="~/ComplianceM.Master" AutoEventWireup="true" CodeBehind="PerformanceReport.aspx.cs" Inherits="ComplianceManager.PerformanceReport" %>
...
<div> 
<div class="heading">
    <asp:Label ID="Label4" runat="server" Text="Departmentwise Performance Report" />
</div>
<asp:Label ID="lblErrorMessage" runat="server" CssClass="error-message" Visible="false" />
    ...
    <div style="flex: 0 0 33%; min-width: 0;">
        <div  class="control-div">
            <asp:Label ID="lblPeriod" runat="server" Text="Period" AssociatedControlID="ddlPeriod" CssClass="control-label" style="width: 60px;" />
            <asp:DropDownList ID="ddlPeriod" runat="server" style="width: 170px;height:20px; font-size:12px;"></asp:DropDownList>
        </div>
        <div style="margin: 0px 0; height: 24px; display: flex; flex-direction: column; gap: 0px;">
            <div style="flex: 1;">
...
<asp:GridView ID="gvCompliances" runat="server" AutoGenerateColumns="false" OnPageIndexChanging="gvCompliances_PageIndexChanging" 
    ...
    <Columns>
        ...
        <asp:TemplateField HeaderText="Total Compliance Requirement" ItemStyle-Width="100px">
            <ItemTemplate>
                <asp:Label ID="lblNoOfCompliance" runat="server" Text='<%# Eval("NoOfCompliance") %>' />
            </ItemTemplate>
        </asp:TemplateField>
        <asp:TemplateField HeaderText="Closed Within Timeline" ItemStyle-Width="160px">
                <ItemTemplate>
                    <asp:Label ID="lblclosedwithinTimeline" runat="server" Text='<%# Eval("closedWithinTimeline") %>' />
                </ItemTemplate>
            </asp:TemplateField>
        <asp:TemplateField HeaderText="Closed With Delays" ItemStyle-Width="70px">
                <ItemTemplate>
                    <asp:Label ID="lblClosedWithDelays" runat="server" Text='<%# Eval("ClosedWithDelays") %>' />
                </ItemTemplate>
            </asp:TemplateField>
        <asp:TemplateField HeaderText="Dellay %age" ItemStyle-Width="75px">
                <ItemTemplate>
                    <asp:Label ID="lblPercentageOfDelays" runat="server" Text='<%# Eval("PercentageOfDelays") %>' />
                </ItemTemplate>
            </asp:TemplateField>
        <asp:TemplateField HeaderText="Pending Surpased Due Date" ItemStyle-Width="75px">
                <ItemTemplate>
                    <asp:Label ID="lblPendingSurpassedDueDate" runat="server" Text='<%# Eval("PendingSurpassedDueDate") %>' />
                </ItemTemplate>
            </asp:TemplateField>
