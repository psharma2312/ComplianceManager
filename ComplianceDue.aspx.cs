...
{
	public partial class ComplianceDue : System.Web.UI.Page
	{
        Int32 userId = 0;
        ...
        private void BindDropdowns()
        {
            try
            {
                ...
                string configValue = ConfigurationManager.AppSettings["DueWithinValues"];

                if (!string.IsNullOrEmpty(configValue))
                {
                    string[] items = configValue.Split(',');

                    ddlDueWithin.Items.Clear();
                    ddlDueWithin.Items.Add(new ListItem("Select Due Within", ""));
                    ...
                    foreach (var item in items)
                    {
                        ...
                        if (parts.Length == 2)
                        {
                            ...
                            ddlDueWithin.Items.Add(new ListItem(text, value));
                        }
                    }
                }
            }
        ...
        protected void gvCompliances_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            ...
            int dueIn = ViewState["Due"] != null ? Convert.ToInt32(ViewState["Due"]) : 0;
            gvCompliances.PageIndex = e.NewPageIndex;
            BindGridFiltered(deptId, typeId, nature, priority, dueIn);
        }
        ...
        protected void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            int due = dropdownValue(ddlDueWithin);
            ...
            {
                ShowMessage("Please select at least one from Department / Nature / Type / Priority / DueWithin.", true);
                return;
            }

            lblErrorMessage.Visible = false;
            BindGridFiltered(deptId, typeOfCompliance, natureOfCompliance, priorityId, due);
        }
        ...
        protected void btnAll_Click(object sender, EventArgs e)
        {
            ...
            ddlDueWithin.SelectedIndex = 0;
            lblErrorMessage.Visible = false;
            gvCompliances.Visible = false;
        }

        private void BindGridFiltered(int deptId = 0, int typeId = 0, int nature = 0, int priority = 0, int dueIn = 0)
        {
            try
            {
                DAL dal = new DAL();
                List<ComplianceMasterr> compliances = new List<ComplianceMasterr>();
                compliances = dal.LoadComplianceDueWithin(deptId, typeId, nature, priority, dueIn);
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            DAL dal = new DAL();
            var varCatalogue = dal.LoadComplianceDueWithin(deptId, typeId, nature, priority, dueIn);
            using (var workbook = new XLWorkbook())
            {
                ...
                string excelName = "ComplianceDueReport_" + localTime.ToString("dd_MMM_yy");
                Response.Clear();
